    ...
    if (action === 'llm_judge') {
      try {
        const verdict = await llmJudge(body.input, {
          model: 'qwen2.5:3b',
          callLLM: async (req) => {
            const resp = await callOllama(req, 'fast');
 ...
 */
function streamOpenAIChatResponse(reply: FastifyReply, response: Record<string, unknown>): FastifyReply {
  ...
  // 2) content chunks (only if we have text content)
  if (content) {
    const STEP = 32;
    for (let i = 0; i < content.length; i += STEP) {
      writeChunk({ content: content.slice(i, i + STEP) });
    }
  }
  ...
  writeChunk({}, content || toolCalls ? 'stop' : 'stop');
...
export async function completionRoute(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/chat/completions', { config: { rateLimit: false } }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (parsed.data.stream) {
      return streamOpenAIChatResponse(reply, response);
    }
