...
const PATTERNS: readonly InjectionPattern[] = [
  ...
  { id: 'pretend-rolemix', category: 'role_bypass', severity: 'high',
    pattern: /\bpretend\s+(?:you\s+)?(?:are\s+not\s+|aren't\s+|don't\s+have\s+|have\s+no\s+)(?:any\s+)?(?:safety\s+|ethical\s+|content\s+|moral\s+)?(?:restrictions?|restraints?|restricted|limits?|limitations?|filters?|guidelines?|guardrails?|rules?|boundaries|bound\s+by|restricted\s+by|limited\s+by|filtered\s+by)\b/i,
