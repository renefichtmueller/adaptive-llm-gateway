/**
 * Core task-oriented client for the Adaptive LLM Gateway.
 *
 * Usage:
 *   import { LLMGatewayClient, createInteractiveClient } from '@llm-gateway/client';
...
export function createInteractiveClient(caller: string, baseUrl?: string): LLMGatewayClient {
  return new LLMGatewayClient({ caller, timeout: 10_000, ...(baseUrl ? { baseUrl } : {}) });
}
...
export function createBatchClient(caller: string, baseUrl?: string): LLMGatewayClient {
  return new LLMGatewayClient({ caller, timeout: 60_000, ...(baseUrl ? { baseUrl } : {}) });
}
...
export function createRealtimeClient(caller: string, baseUrl?: string): LLMGatewayClient {
  return new LLMGatewayClient({ caller, timeout: 5_000, ...(baseUrl ? { baseUrl } : {}) });
}
